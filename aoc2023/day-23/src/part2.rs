...
fn dfs(grid: &Vec<Vec<char>>, starting_position: (usize, usize), ending_position: (usize, usize), dirs: Vec<(i32, i32)>) {
    ...
    while !stack.is_empty() && path_count > 0{
        ...
        for (dy, dx) in dirs.iter(){
            ...
            //     false
            // };
            let can_step = is_valid_neighbour_coordinate(&grid, ny, nx);
...
// fn bfs(grid: &Vec<Vec<char>>, starting_position: (usize, usize), ending_position: (usize, usize), dirs: Vec<(i32, i32)>){
...
//     pathx.push((0, starting_position));
//     while !q.is_empty(){
//         let (depth , (top_y, top_x)) = q.pop_front().unwrap();
//         visited.insert((top_y as usize, top_x as usize));

//         if top_y == ending_position.0 && top_x == ending_position.1{
//             dbg!(depth);
//         }
...
//                 path.push((ny as usize, nx as usize));
//                 pathx.push((depth + 1, (ny as usize, nx as usize)));
//             }
